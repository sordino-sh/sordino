#!/usr/bin/env bash
# Backs /zlauder:uninstall — REMOVES the ZlauDeR plumbing (the inverse of enable.sh).
# Strips env.ANTHROPIC_BASE_URL (and env.ZLAUDER_PORT) from the project's
...
# .claude/zlauder-statusline.json; if the slot was empty, our line is dropped. Our
# permissions.deny/ask rules and autoMode.environment note are removed too. Every other
# setting is preserved; the file is rewritten atomically. The seeded zlauder.toml (your
# masking POLICY) is deliberately LEFT in place — uninstall removes the plumbing, not your
# config. Turning masking off (without removing anything) is /zlauder:disable instead.
...
# Resolve the zlauder-hooks binary like every other script, then hand the settings edit to
...
# ZLAUDER_PORT (and an emptied env), removes our permissions/autoMode entries, restores the
...
if ! command -v "$ZLAUDER_HOOKS_BIN" >/dev/null 2>&1; then
  ...
  echo "  Start a Claude Code session in this project (or put the binaries on PATH), then re-run /zlauder:uninstall." >&2
...
# `/zlauder:uninstall --all` sweeps EVERY plumbed project (run this BEFORE removing the
...
case "$rc" in
  0)
    ...
    echo "Your masking policy (zlauder.toml) was left in place. To remove ZlauDeR from EVERY project before deleting the plugin, run /zlauder:uninstall --all."
    ;;
  3)
    echo "ZlauDeR already removed: no ZlauDeR wiring in this project."
    ;;
  *)
    exit "$rc"
    ;;
