...
Codex plugin package for Sordino's OpenAI proxy path. The `enable` skill starts
...
`$CODEX_HOME/config.toml`: a custom `[model_providers.sordino]` provider block
pointed at the live proxy plus a top-level `model_provider = "sordino"`. You do
...
The bundled `sordino.toml` is a starter seed. Put a project-specific
`sordino.toml` at the project root to override it.
