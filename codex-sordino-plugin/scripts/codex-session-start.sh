...
# exec `sordino-hooks codex-session-start`, passing the SessionStart payload on
...
if ! { [ -r "$SCRIPT_DIR/_resolve-bins.sh" ] && . "$SCRIPT_DIR/_resolve-bins.sh"; }; then
  printf '%s\n' "Sordino: could not source resolver; SessionStart onboarding skipped." >&2
  printf '{}\n'
  exit 0
fi

if ! sordino_resolve_bins; then
  printf '%s\n' "Sordino: could not resolve sordino-hooks; SessionStart onboarding skipped." >&2
  printf '{}\n'
  exit 0
fi

exec "${SORDINO_HOOKS_BIN:-sordino-hooks}" codex-session-start
