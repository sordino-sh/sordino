---
name: sordino-openai
description: Use when working in a Codex project that routes OpenAI traffic through Sordino for local PII masking.
---
...
- Routing is enabled by the `enable` skill writing `$CODEX_HOME/config.toml`: a
  custom `[model_providers.sordino]` provider block + `model_provider = "sordino"`.
