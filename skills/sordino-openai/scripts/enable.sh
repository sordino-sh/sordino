#!/usr/bin/env bash
# Route Codex's OpenAI traffic through the sordino masking proxy.
#
# Thin wrapper: (1) auth preflight, (2) learn the live proxy URL, (3) write the
# custom-provider block into $CODEX_HOME/config.toml via `sordino-hooks codex-config enable`.
# Diagnostics go to stderr; the only stdout is the human instruction at the end.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Put sordino-hooks (and the proxy) on PATH via the shared resolver, if present.
...
if [ -f "$SCRIPT_DIR/../../../scripts/_resolve-bins.sh" ]; then
  ...
  sordino_resolve_bins || {
    printf '%s\n' "Sordino: could not resolve sordino-hooks; routing not enabled." >&2
    exit 1
  }
fi
...
if ! "$HOOKS" codex-auth-check --json; then
  printf '%s\n' "Sordino: Codex auth preflight failed — no config written. Export a usable OPENAI_API_KEY (sk-...) and retry." >&2
  exit 1
fi
...
if [ -z "${BASE_URL:-}" ]; then
  printf '%s\n' "Sordino: could not bring the masking proxy up — no config written." >&2
  exit 1
fi
URL="${BASE_URL%/}/v1"
...
if [ "$rc" -eq 3 ]; then
  printf '%s\n' "Sordino: Codex routing already enabled (proxy at $URL)."
else
  printf '%s\n' "Sordino: Codex routing enabled — masking proxy at $URL."
fi
