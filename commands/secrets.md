---
description: View Sordino registered-secret status for this project — the readiness gate and which secrets resolved (registered by reference; values are never shown)
...
`pass`/`age`/`sops`/`dotenv`/`env`) — Sordino resolves the value at startup and masks
