---
description: Show Sordino proxy health, whether this session is routed, and the masking state (on/off, profile, categories, ML) for this project — read-only.
argument-hint: ""
...
The block above is Sordino's read-only status for **this project only**: proxy health,
...
ML model, reveal a token, or scrub a transcript — the user runs `/sordino:privacy …`
themselves (`/sordino:disable` to quickly turn masking off; `/sordino:enable` /
`/sordino:uninstall` for routing). You may **suggest the
