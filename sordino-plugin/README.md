# sordino (Claude Code plugin)

Control plane for **Sordino**, a local PII mask/unmask reverse-proxy for Claude
...
reveals a token for audit. The masking itself lives in the `sordino-proxy` /
`sordino-engine` binaries; see the [main repo README](../README.md) for the full
design.
...
`SORDINO_PORT` + a `🛡` status line) into this project's
...
Sordino's intake gate blocks its messages so nothing sends unmasked first. (Web
...
2. **Restart Claude Code once.** A freshly-written route is only read reliably at
   ...
   `⟳ Sordino: restart to mask` until then). Until this session is routed, Sordino
   blocks outbound messages so nothing sends unmasked first — and every session after
   the first is masked automatically. (`/sordino:enable` does the same write explicitly
...
pick `project` if you'd rather load sordino in only one repo.
(Or add this directory as a local marketplace if you are working in-repo.)
Installing the plugin wires up the `SessionStart` hook and the `/sordino:*`
...
prompt on its edits of `sordino.toml` / `sordino.local.toml`).

> **Defense-in-depth, not a sandbox.** These rules block the casual and prompt-injection
> paths (the model running `/sordino:privacy off`, the `sordino-*` CLIs, or editing the config
...
> **Changed in 0.2.0:** `/sordino:status` and `/sordino:reveal` were folded into
> `/sordino:privacy` (`/sordino:privacy status` and `/sordino:privacy reveal <token>`).
> The standalone `sordino-hooks init` CLI setup path was removed — the plugin is now
> the sole install interface.
>
> **Changed in 1.0:** `/sordino:status` is back as a standalone **read-only** command so the
> model can check masking state without being able to change it; `/sordino:privacy` (which
> can *loosen* masking) is now user-only. `reveal` / `scrub` stay inside the user-only
> `/sordino:privacy`.

## Troubleshooting

- **"Is masking actually on?"** Run `/sordino:verify`. It checks two independent things — the
  engine **masks** (a key-gated canary) AND this session **routes** through the proxy; both must
  pass. `/sordino:status` shows the same at a glance via the status line.
- **A message was blocked, or the status line shows `⟳ Sordino: restart to mask`.** Routing was
  just written, but Claude Code applies a new route reliably only at startup — so until you
  restart, Sordino's **intake gate blocks this session's messages** (it would otherwise send
  them UNMASKED before masking is live). **Restart Claude Code once**, then `/sordino:verify`.
  To send a message this session WITHOUT masking — skipping the gate — set
  `SORDINO_NO_INTAKE_GATE=1` in your environment before launching Claude Code.
  (`/sordino:enable` re-writes the route if it went missing.)
- **Requests hang for ~3 minutes, or the status line shows `⚠ Sordino routed, proxy down`.** A
  local security/AV product or a hardened loopback firewall is intercepting `127.0.0.1`. Run
  `/sordino:doctor` — it pinpoints the loopback / firewall / port problem and prints the fix.
- **A token won't decode with `/sordino:privacy reveal`.** "unknown token" means no such handle
  ...
  tool boundary — by design; use `/sordino:monitor` to inspect it locally.
- **Every request fails right after deleting the plugin.** A project's `settings.local.json`
  still points at a now-dead proxy. Run **`/sordino:uninstall --all` BEFORE deleting the plugin**
  to sweep every plumbed project; if you already deleted it, run `/sordino:uninstall` in the
  affected project (or delete the `ANTHROPIC_BASE_URL` / `SORDINO_PORT` lines from its
  `.claude/settings.local.json`).
- **See what's being masked.** `/sordino:monitor` prints a local, key-gated URL to inspect this
  project's masked traffic; `/sordino:secrets` shows which registered `[[secrets]]` resolved.
...
`sordino-proxy` and `sordino-hooks` ship **prebuilt, per-platform** with the
...
`/plugin install sordino` lands a ready-to-run binary for your OS/arch with **no
...
1. **On `PATH`** — an already-installed `sordino-proxy` / `sordino-hooks`.
...
6. **Build from source** (in-repo dev only) — `cargo build --release` from
   `$SORDINO_WORKSPACE` or `${CLAUDE_PLUGIN_ROOT}/..`, cached into
   ...
   for someone hacking on sordino itself.
...
(`x86_64-pc-windows-msvc`). Release assets are named `sordino-<triple>.tar.gz`
for Linux/macOS and `sordino-x86_64-pc-windows-msvc.zip` for Windows. On Windows,
...
### Invocation shapes Sordino's hooks cannot see

Sordino ships entirely as this plugin's `SessionStart` / `UserPromptSubmit` /
...
- **`claude --bare` / `--safe-mode`** (env `CLAUDE_CODE_SIMPLE=1` /
  ...
  partial skip: `loadPluginHooks()` simply never runs, so Sordino's fail-closed
  ...
  `sordino:doctor` can only detect this from *within* a normal session
  (reading its own `CLAUDE_CODE_SIMPLE`/`CLAUDE_CODE_SAFE_MODE` env) — it
  cannot self-detect from inside a `--bare` session, since Sordino's own hook
  wouldn't be invoked there either. Avoid `--bare`/`--safe-mode` on projects
  where you're relying on Sordino, and check your shell/CI environment isn't
...
- **`claude mcp serve`** (Claude Code exposing its own Bash/Read/Edit/etc. tool
  ...
  execute directly with zero permission decision and zero Sordino
  ...
  ungated local tool access with none of Sordino's protections in effect.
- **Remote Control's `bash_command` message** (the transport behind
  ...
  with zero Sordino involvement, reachable from wherever Remote Control is
  paired.
