---
description: View or change Sordino ZDR (Zero-Data-Retention) trusted routing for THIS session — status, on/off, and the configured targets. Optional; off unless you configure [zdr] targets.
...
`[[zdr.target]]` entries in `sordino.toml`.
...
- ZDR is the **user's assertion**, not something Sordino can verify. There is no way
...
- Never print or echo any credential, the session/control key, or claim a value was
  revealed. Configuring targets is done by editing `sordino.toml` `[[zdr.target]]`
