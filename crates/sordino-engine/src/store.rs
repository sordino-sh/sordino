...
impl SessionStore {
    ...
    /// token-grammar-safe handle (today `Local` is bound to the clean `sordino_admin_key`,
...
mod tests {
    ...
    fn class_of_peeks_kind_and_name_without_decrypting() {
        ...
        let local = s.intern_local("SORDINO_ADMIN_KEY", "k").unwrap();
    ...
    fn local_token_reveals_as_pii_path_no_then_local_yes() {
        let mut s = SessionStore::new();
        let tok = s.intern_local("SORDINO_ADMIN_KEY", "adminval").unwrap();
        assert!(tok.starts_with("[SORDINO_ADMIN_KEY_"), "got {tok}");
