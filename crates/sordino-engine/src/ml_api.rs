...
//!
//! The presidio `Recognizer` trait is infallible (a backend error becomes an
//! empty result = "no PII" = a leak), so the engine stores this FALLIBLE trait
//! instead: an `Err` refuses the request. Always compiled (no feature gate) so
//! the ML slot's type exists even in a regex-only build (e.g. `sordino-hooks`,
//! which then never pulls `presidio-classifier`); only the backends behind it
//! are feature-gated.
//!
//! The production recognizer (`presidio_classifier::TokenClassifierRecognizer`,
//! both Candle and HTTP) implements [`MlRecognizer`] directly in
//! [`crate::ml`] by driving its inherent fail-CLOSED `try_analyze` under a panic
//! guard. [`InfallibleMl`] is the adapter for INFALLIBLE recognizers — the test
//! mocks that implement `presidio_core::Recognizer` — and is fail-SOFT (it calls
//! the swallowing `analyze`), so it must not wrap a real fallible backend.
...
/// Run a recognizer call with panic containment. Shared with [`crate::ml`]'s
/// `TokenClassifierRecognizer` impl, which wraps the upstream `try_analyze`
/// (that seam surfaces `Result` errors but does NOT catch panics).
pub(crate) fn catch_recognizer_panic<T>(f: impl FnOnce() -> T) -> Result<T, EngineError> {
