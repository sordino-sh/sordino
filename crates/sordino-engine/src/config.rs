...
//
// These sordino-local `Custom` entity types are NOT canonical `presidio_core::EntityType`
    ...
    pub fn entity_types(self) -> &'static [&'static str] {
        match self {
            Category::Secrets => &[
                ...
                "PRIVATE_KEY",
                "JWT",
                // sordino-local Custom entity (context-based recognizer). Kept in the
            ...
            Category::Financial => &[
                ...
                "US_BANK_NUMBER",
                "ABA_ROUTING_NUMBER",
                // sordino-local Custom entities (hard-context regex recognizers).
            ...
            Category::Identity => &[
                ...
                "UK_NHS",
                "UK_NINO",
                // sordino-local Custom entities. `DATE_OF_BIRTH` is the hard-context
...
impl AllowList {
    ...
    pub fn with_common_words() -> Self {
        ...
        // Claude Code / sordino self-reference false positives: the harness's own
        ...
        // regex — "Claude Sonnet 5", "Sordino", the literal token-format example
        ...
        al.add_exact_ci("sordino");
        // The literal example token handle used in sordino's own docs/prompts, in each
        ...
        // The one fixed, non-personal Anthropic address sordino's own commit template
...
pub struct MlConfig {
    ...
    /// child binary. `None` ⇒ resolve from the `SORDINO_BURN_SERVER` env override,
...
pub enum MlBackend {
    ...
    /// port, no socket. The child owns its own cubecl/wgpu device, so sordino's
...
/// The HuggingFace repo ids Sordino is authorized to fetch + load — an allowlist,
...
/// supply-chain / RCE vector. Pinning here means a model-supplied (`/sordino:privacy
...
impl EngineConfig {
    ...
    ///
    /// THIRD known-key source — sordino-local Custom entities in a CATEGORY. The four
    ...
    pub fn unknown_entity_types(&self) -> Vec<String> {
        ...
        self.entity_operators
            ...
            .filter(|k| {
                ...
                // A sordino-local Custom entity that lives in a category (e.g. CVV,
    ...
    /// (sordino's everything-configurable contract), with a documented PCI-SAD warning.
    ...
    pub fn detection_fingerprint(&self) -> u64 {
        ...
        h.update(b"sordino-policy-fp-v1");
...
mod tests {
    ...
    fn common_words_allow_list_covers_claude_code_self_reference() {
        ...
        // sordino self-reference, any case.
        for v in ["sordino", "Sordino", "Sordino", "SORDINO"] {
            assert!(al.is_allowed(v), "expected allowed: {v:?}");
        }
        ...
        assert!(!al.is_allowed("sordinoish"));
    ...
    fn same_model_params_uses_backend_specific_identity() {
        ...
        let sidecar = MlConfig {
            ...
            sidecar_path: Some("/opt/sordino/burn-server".into()),
            ..lax.clone()
        };
        ...
        let sidecar_other_binary = MlConfig {
            sidecar_path: Some("/opt/sordino/burn-server-v2".into()),
            ..sidecar.clone()
        };
        assert!(
    ...
    // --- C3/C4/C8: sordino-local Custom entity contract ---------------------
    ...
    fn unknown_entity_types_accepts_custom_entities_but_flags_typo() {
        // The four sordino-local Custom entities are valid entity_operators keys (they
