...
mod tests {
    ...
    async fn missing_binary_is_binary_missing() {
        let err = SecretBroker::run(&["sordino-no-such-binary-xyz".into()], None)
            .await
            .unwrap_err();
