...
//!
//!   cargo run -p zlauder-engine --example profile_bench
...
fn hard_negatives() -> Vec<(&'static str, &'static str)> {
    vec![
        (
            "abs-path",
            "/home/user/Projects/zlauder/crates/zlauder-engine/src/detect.rs",
        ),
        (
            "kebab-ident",
...
fn masked_expected(out: &zlauder_engine::MaskOutcome, p: &Positive, original: &str) -> bool {
...
fn main() {
    ...
    println!("== zlauder profile detection benchmark (ML OFF) ==");
    println!(
