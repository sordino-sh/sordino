//! Shared on-disk session state for sordino.
//!
//! `sordino-proxy` and `sordino-hooks` both need to agree on, per project, the
//! proxy's port, its admin key (the `x-sordino-key` for the audit/control
...
pub const BUILD_ID: &str = match option_env!("SORDINO_BUILD") {
    Some(s) => s,
    None => "unknown",
};

/// The user-scope config path: `$SORDINO_USER_CONFIG`, else
/// `$XDG_CONFIG_HOME/sordino/config.toml`, else `$HOME/.config/sordino/config.toml`.
/// Shared by the proxy (layer loading) and the hooks CLI (`--scope user` edits) so
/// they agree on the location.
pub fn user_config_path() -> PathBuf {
    if let Some(p) = std::env::var_os("SORDINO_USER_CONFIG") {
    ...
    let base = std::env::var_os("XDG_CONFIG_HOME")
        ...
        // path becomes %APPDATA%\sordino\config.toml.
        .or_else(windows_appdata)
        .unwrap_or_else(|| PathBuf::from(".config"));
    base.join("sordino").join("config.toml")
}

/// Root directory for sordino state files (created `0700` on Unix).
///
/// `SORDINO_STATE_DIR` wins; else `$XDG_RUNTIME_DIR/sordino`; else (Windows)
/// `%LOCALAPPDATA%\sordino`; else a temp dir.
///
...
pub fn state_dir() -> Result<PathBuf> {
    let base = std::env::var_os("SORDINO_STATE_DIR")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("XDG_RUNTIME_DIR").map(|d| PathBuf::from(d).join("sordino")))
        .or_else(windows_localappdata_sordino)
        .unwrap_or_else(|| std::env::temp_dir().join("sordino"));
    std::fs::create_dir_all(&base).with_context(|| format!("creating state dir {base:?}"))?;
    set_mode(&base, 0o700);
    Ok(base)
}

/// `%LOCALAPPDATA%\sordino` on Windows (a per-user, non-volatile dir), else `None`.
#[cfg(windows)]
fn windows_localappdata_sordino() -> Option<PathBuf> {
    std::env::var_os("LOCALAPPDATA").map(|d| PathBuf::from(d).join("sordino"))
}
#[cfg(not(windows))]
fn windows_localappdata_sordino() -> Option<PathBuf> {
    None
...
/// project the user opted out of, and `/sordino:uninstall --all` uses it to sweep every
/// plumbed project's routing before removing the plugin.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum PlumbState {
    /// sordino auto-plumbed (or the user enabled) routing for this project.
    Plumbed,
    /// The user ran `/sordino:uninstall` here — never auto-plumb it again.
    Optout,
}
...
/// sordino has never plumbed or seen it.
...
/// Every project root currently in the `Plumbed` state (used by `/sordino:uninstall --all`).
...
// (hook→proxy via `SORDINO_LAUNCH_NONCE`, echoed on `/healthz`) lets the launcher
...
pub struct Rendezvous {
    ...
    pub port: u16,
    /// Hex of the proxy's control token (`x-sordino-key`). FRESHLY MINTED by the
    ...
    /// matches the nonce it handed the proxy via `SORDINO_LAUNCH_NONCE`.
    #[serde(default)]
    pub nonce: String,
}
    ...
    pub fn message(self, port: u16, bind: &str) -> String {
        match self {
            BindFault::InUse => format!(
                "port {port} is already in use — another process (or a stale sordino proxy) holds it. \
                 Pick a different `[proxy] port` in sordino.toml, or remove it to use an OS-assigned \
...
/// `SORDINO_STATE_DIR` / `SORDINO_USER_CONFIG` env vars. Shared across BOTH test
...
mod rendezvous_tests {
    ...
    // The shared env lock (see TEST_ENV_LOCK) — SORDINO_STATE_DIR is process-global.
    use super::TEST_ENV_LOCK as ENV_LOCK;

    fn with_state_dir(tag: &str, f: impl FnOnce(&std::path::Path)) {
        let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("sordino-{tag}-{}", std::process::id()));
...
mod tests {
    ...
    // The shared env lock (see TEST_ENV_LOCK): SORDINO_STATE_DIR/SORDINO_USER_CONFIG are
    ...
    fn registry_round_trips_and_filters_plumbed() {
        let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("sordino-reg-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        // SAFETY: single-threaded test; points the registry at a temp config dir.
        unsafe { std::env::set_var("SORDINO_USER_CONFIG", dir.join("config.toml")) };
