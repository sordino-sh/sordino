...
async fn responses_inner(st: AppState, req: Request, conversation: Option<String>) -> Response {
    ...
    // Bare-path defense-in-depth: a bare `/v1/responses` carrying x-sordino-conversation
    // naming a LIVE pin must refuse (409) rather than route to the default endpoint.
    // NON-consuming header read BEFORE into_parts; runs only when the URL has no session
    // id (precedence: URL id > header id > none).
    if let Some(resp) =
        routes::bare_path_zdr_guard(&st, &req, conversation.as_deref(), "/v1/responses")
    {
        return resp;
    }
...
mod tests {
    use super::*;
    use sordino_engine::{EngineConfig, RevealMarker};
