//! sordino-proxy — local reverse proxy that masks PII in Claude Code's
...
//! pid, so the `sordino-hooks` CLI always reaches a key that matches the live proxy —
...
#[derive(Parser, Debug)]
#[command(name = "sordino-proxy", version, about)]
struct Args {
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "sordino_proxy=info,warn".into()),
        )
        .init();
    ...
    let launch_nonce = std::env::var("SORDINO_LAUNCH_NONCE").unwrap_or_default();
    ...
    for e in &resolved_zdr.errors {
        tracing::warn!("sordino ZDR config: {e}");
    }
    if !resolved_zdr.targets.is_empty() {
        tracing::info!(
            "sordino: {} ZDR target(s) registered ({}); routing-only, masking still applies",
    ...
    if !broker_allows.is_empty() {
        match proxy_secrets::build_broker_policy(&broker_allows) {
            ...
            Err(e) => tracing::error!(
                "sordino: invalid [broker] policy ({e}); broker resolution DISABLED (default-deny)"
            ),
        }
    }
    let (_key, salt) = engine.session_handle();
    let salt_hex = hex_encode(&salt);
    // The `x-sordino-key` is a control token DERIVED from the AES key (blake3), not
    ...
    for outcome in &reload_report.outcomes {
        match outcome {
            sordino_proxy::state::ReloadOutcome::Restored {
                conversation,
                target,
            } => tracing::info!(
                "sordino ZDR: restored selection for conversation {conversation} -> {target}"
            ),
            sordino_proxy::state::ReloadOutcome::Reverted {
                conversation,
                reason,
            } => tracing::warn!(
                "sordino ZDR: reverted conversation {conversation} to Normal ({reason})"
            ),
        }
    }
    ...
    if let Err(e) = sordino_state::write_rendezvous(&sordino_state::Rendezvous {
        project_root,
        port,
        admin_key,
        salt: salt_hex,
        ...
        bind,
        last_port: port,
        build_id: sordino_state::BUILD_ID.to_string(),
        started_unix: sordino_state::now_unix(),
        nonce: launch_nonce,
    }) {
        tracing::warn!("could not write rendezvous record (CLI may not find the proxy): {e}");
    }

    tracing::info!("sordino-proxy listening on http://{addr} -> {upstream}");
    ...
    if !secret_specs.is_empty() {
        tokio::spawn(async move {
            let registry =
                sordino_secrets::default_registry(Some(PathBuf::from(&proot_for_secrets)));
            ...
            if all_ok {
                ...
                tracing::info!("sordino: {resolved}/{total} secret(s) resolved; intake open");
            } else {
                tracing::warn!(
                    "sordino: required secret(s) unresolved ({resolved}/{total}); LLM intake held at 503"
                );
            }
        });
    }
    ...
    if let Err(e) = sordino_state::rendezvous_remove_if_owned(
        &shutdown_root,
        &shutdown_nonce,
        std::process::id(),
    ) {
...
fn build_engine(cfg: EngineConfig) -> anyhow::Result<MaskEngine> {
    match std::env::var("SORDINO_SESSION_SALT").ok() {
        Some(s) => {
            let salt = decode_hex_array::<16>(&s).context("SORDINO_SESSION_SALT (need 32 hex)")?;
...
async fn download_model(mut ml: MlConfig, model_override: Option<String>) -> anyhow::Result<()> {
    ...
    match ml.backend {
        sordino_engine::MlBackend::Http => println!(
            "Sordino: [engine.ml] backend = \"http\" — nothing to download; probing \
             the endpoint {} instead.",
            ml.endpoint.as_deref().unwrap_or("(unset)")
        ),
        sordino_engine::MlBackend::Sidecar => println!(
            "Sordino: [engine.ml] backend = \"sidecar\" — spawning the burn-wgpu child \
        ...
        sordino_engine::MlBackend::Local => println!(
            "Sordino: downloading ML model '{}' for CPU inference. The first run can be \
    ...
    let res = tokio::task::spawn_blocking(move || sordino_engine::ml::download(&ml))
        .await
        .context("download task panicked")?;
    match res {
        Ok(()) => {
            println!(
                "Sordino: model '{model}' ready (cached / endpoint probed). Enable it \
                 with `/sordino:privacy model on`."
            );
            Ok(())
        }
