...
//!
//! Precedence (later wins): user (`~/.config/sordino/config.toml`) <
//! project (`./sordino.toml`, the `--config` path) < local
//! (`<project-dir>/sordino.local.toml`, gitignored). The `/privacy` CLI persists
//! to one of these files per `--scope`, and the proxy re-reads + re-merges them on
//! `POST /sordino/reload`. A per-key override replaces wholesale (a project's
...
use sordino_engine::{AllowList, EngineConfig, Profile};
...
pub struct ZdrSection {
    /// The target `/sordino:zdr` engages when given no explicit config name.
    #[serde(default)]
...
/// argument, usually `./sordino.toml`).
pub fn resolve_layers(project: Option<&Path>) -> ConfigLayers {
    let local = project.map(|p| {
        p.parent()
            .unwrap_or(Path::new("."))
            .join("sordino.local.toml")
    });
    ConfigLayers {
        user: sordino_state::user_config_path(),
        ...
        if let Some(tbl) = item.as_table() {
            for forbidden in ["value", "literal", "secret", "plaintext"] {
                if tbl.contains_key(forbidden) {
                    anyhow::bail!(
                        "sordino config: secrets[{i}] has a `{forbidden}` key — secret VALUES \
        ...
        if let Some(tbl) = item.as_table() {
            for (k, _) in tbl {
                ...
                if FORBIDDEN.contains(&k.to_ascii_lowercase().as_str()) {
                    anyhow::bail!(
                        "sordino config: zdr.target[{i}] has a `{k}` key — a ZDR credential VALUE \
            ...
            if let Some(hdrs) = tbl.get("extra_headers").and_then(toml::Value::as_table) {
                for hk in hdrs.keys() {
                    ...
                    if crate::zdr::header_name_is_auth_bearing(hk) {
                        anyhow::bail!(
                            "sordino config: zdr.target[{i}].extra_headers has a credential-bearing \
...
/// The interactive `PUT /sordino/config` REJECTS the same condition (admin.rs); this
...
fn warn_unknown_entity_types(engine: &EngineConfig) {
    ...
    if !unknown.is_empty() {
        eprintln!(
            "Sordino: WARNING — config has entity_operators key(s) {unknown:?} that are not \
...
fn warn_narrowing_profile(merged: &toml::Value, engine: &EngineConfig) {
    ...
    eprintln!(
        "Sordino: NOTE — config sets `profile = \"{profile_name}\"` without an explicit \
...
/// the caller); `Project`/`Local` fall back to `<project_root>/sordino.{,local.}toml`
...
fn scope_file_path(layers: &ConfigLayers, project_root: &str, scope: Scope) -> Option<PathBuf> {
    match scope {
        ...
        Scope::Project => layers.project.clone().or_else(|| {
            (!project_root.is_empty()).then(|| Path::new(project_root).join("sordino.toml"))
        }),
        Scope::Local => layers.local.clone().or_else(|| {
            (!project_root.is_empty()).then(|| Path::new(project_root).join("sordino.local.toml"))
        }),
    }
}
...
/// hooks CLI's `apply_profile` writes, so the proxy `POST /sordino/profile/{name}`
...
mod tests {
    use super::*;
    use sordino_engine::{Category, Operator};

    #[test]
    fn loads_repo_sordino_toml() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../sordino.toml");
        let cfg = load(Some(&path)).expect("sordino.toml should parse");
    ...
    #[test]
    fn loads_repo_sordino_toml_example() {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../sordino.toml.example");
        let cfg = load(Some(&path)).expect("sordino.toml.example should parse");
    ...
    #[test]
    fn loads_plugin_seed_sordino_toml() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../sordino-plugin/sordino.toml");
        let cfg = load(Some(&path)).expect("plugin sordino.toml should parse");
    ...
    #[test]
    fn loads_plugin_sordino_toml_example() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../sordino-plugin/sordino.toml.example");
        let cfg = load(Some(&path)).expect("plugin sordino.toml.example should parse");
    ...
    fn missing_config_uses_defaults() {
        ...
        unsafe { std::env::set_var("SORDINO_USER_CONFIG", "/nonexistent/sordino/config.toml") };
    ...
    fn parses_engine_ml_section() {
        let dir = std::env::temp_dir().join(format!("sordino-ml-{}", std::process::id()));
    ...
    fn parses_engine_reveal_marker_section() {
        let dir = std::env::temp_dir().join(format!("sordino-marker-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let project = dir.join("sordino.toml");
    ...
    fn parses_zdr_section() {
        let dir = std::env::temp_dir().join(format!("sordino-zdr-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let project = dir.join("sordino.toml");
        std::fs::write(
            &project,
    ...
    fn rejects_inline_zdr_credential() {
        let dir = std::env::temp_dir().join(format!("sordino-zdrbad-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let project = dir.join("sordino.toml");
    ...
    fn rejects_auth_bearing_zdr_extra_header() {
        let dir = std::env::temp_dir().join(format!("sordino-zdrhdr-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let project = dir.join("sordino.toml");
    ...
    fn rejects_keyword_named_zdr_extra_header_even_with_plain_value() {
        let dir = std::env::temp_dir().join(format!("sordino-zdrkw-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let project = dir.join("sordino.toml");
    ...
    fn local_layer_overrides_project() {
        let dir = std::env::temp_dir().join(format!("sordino-layer-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let project = dir.join("sordino.toml");
