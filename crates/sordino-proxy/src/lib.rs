//! sordino-proxy library surface: request masking, response unmasking, routing.
//! The binary (`main.rs`) is a thin wrapper around these modules.

// The proxy's ML lifecycle (`ml.rs`, `--download-model`) calls into
// `sordino_engine::ml`, which only exists when at least one backend is compiled
...
// unresolved-`sordino_engine::ml` errors would drown it.
#[cfg(not(any(feature = "ml", feature = "ml-http", feature = "ml-sidecar")))]
compile_error!(
    "sordino-proxy needs at least one ML backend feature: build with `--features ml` \
