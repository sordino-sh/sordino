...
impl Monitor {
    ...
    /// liveness or config-on-disk. A `-c`/`-p` provider override (config says sordino
    ...
    /// payload is the `GET /sordino/config` JSON (`{ config, ml, … }`). Called by
    ...
    /// `/sordino:privacy` CLI, a profile apply, an ML toggle, or a file reload.
...
/// supplied id source — the `x-sordino-conversation` header, the session URL segment, and
...
mod tests {
    ...
    fn explicit_conversation_id_is_bounded() {
        // The higher-precedence explicit id (x-sordino-conversation header / session URL
    ...
    fn manifest_cvv_and_email() -> UnmaskManifest {
        ...
        m.push(sordino_engine::ManifestEntry {
            ...
            entity_kind: sordino_engine::ENTITY_CVV.to_string(),
            arrow_origin: sordino_engine::Surface::UserMessage,
            exposed_at: None,
            broker: false,
            local: false,
    ...
    fn cross_turn_revealed_local_scrubbed_via_session_set() {
        ...
        m.set_local_redactions(vec![(
            ...
            "[SORDINO_ADMIN_KEY_aabbccdd]".to_string(),
        )]);
        ...
        assert!(
            preview.contains("[SORDINO_ADMIN_KEY_aabbccdd]"),
    ...
    fn session_token_ledger_is_capped() {
        ...
        for i in 0..(MAX_SESSION_TOKENS + 50) {
            man.push(sordino_engine::ManifestEntry {
    ...
    fn session_token_eviction_drops_oldest_first() {
        ...
        for i in 0..(MAX_SESSION_TOKENS - 10) {
            old.push(sordino_engine::ManifestEntry {
        ...
        for i in 0..50 {
            fresh.push(sordino_engine::ManifestEntry {
    ...
    fn cvv_manifest_entry_is_non_peekable_and_value_withheld() {
        let m = Monitor::new();
        // entity_kind "CVV" matches sordino_engine::ENTITY_CVV (the recognizer's stamp).
    ...
    fn session_token_reused_value_survives_eviction() {
        ...
        for i in 0..(MAX_SESSION_TOKENS - 1) {
            fill.push(sordino_engine::ManifestEntry {
        ...
        for (handle, value) in [("[REUSED]", "s3cr3t"), ("[FRESH]", "new")] {
            reuse.push(sordino_engine::ManifestEntry {
                canonical_form: value.to_string(),
    ...
    fn manifest_entry(handle: &str, value: &str, kind: &str) -> sordino_engine::ManifestEntry {
        sordino_engine::ManifestEntry {
            canonical_form: value.to_string(),
            token_handle: handle.to_string(),
