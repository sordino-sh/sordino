...
//! `/sordino/monitor/snapshot` and the SSE `/events` stream. Integration tests
...
use sordino_engine::{ENTITY_CVV, ManifestEntry};
...
pub(crate) enum MonitorEvent {
    ...
    /// `GET /sordino/config` (`{ config, ml, … }`). Pushed by every control-plane
    /// writer (UI PUT, `/sordino:privacy` CLI, profile, ml toggle, reload) so any
...
/// and from the persisted `sordino.local.toml`.
#[derive(Deserialize)]
